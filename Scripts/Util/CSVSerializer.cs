...
{
    static public T[] DeserializeData<T>(string text)
    {
    ...
    static void SetValue(object v, FieldInfo fieldinfo, string value)
    {
        ...
        if (fieldinfo.FieldType.IsArray) // Array
        {
        ...
        else if (fieldinfo.FieldType.IsEnum) // Enum
        {
...
//        }
//#endif
        else if (fieldinfo.FieldType == typeof(string)) // string
        {
            fieldinfo.SetValue(v, value);
        }
        else if(fieldinfo.FieldType == typeof(Vector3)) //Vector3
        {
            Vector3 vec = Vector3.zero;
            float[] elements = value.Split(',').Select(x => float.Parse(x)).ToArray();
            if(elements != null && elements.Length == 3)
            {
                vec = new Vector3(elements[0], elements[1], elements[2]);
            }
            fieldinfo.SetValue(v, vec);
        }
        else if (fieldinfo.FieldType == typeof(Quaternion))
        {
            ...
            if (elements != null && elements.Length == 3)
            {
                quat = Quaternion.Euler(elements[0], elements[1], elements[2]);
            }
            fieldinfo.SetValue(v, quat);
        }
        else
